...
>    Пример `cooler` расположен в директории `examples/cooler`.
>  * Отдельный архив [cooler.zip](https://download-directory.github.io/?url=https%3A%2F%2Fgithub.com%2Ftarantool%2Ftarantooldb-examples%2Ftree%2Frelease-3x%2Fmaster%2Fexamples%2Fcooler&filename=cooler), скачанный из этого репозитория.
...
В этой миграции создан на движке memtx [шардированный спейс](https://www.tarantool.io/docs/tdb/ru/3_x/admin_guide/sharding) `sessions` для хранения пользовательских сессий:
...
Также в миграции, [описанной выше](#создание-спейса-в-memtx), указаны [настройки процесса архивации](#создание-спейса-vinyl)
для заданного спейса memtx и определено [условие](#условие-архивации), по которому будет запускаться архивация.

### Создание спейса vinyl

Метод [cooler.setup()](https://www.tarantool.io/docs/tdb/ru/3_x/reference/api_reference/cooler#reference_lua-cooler-setup) настраивает процесс архивации для заданного спейса memtx.
...
Функцию с условием архивации можно задать с помощью метода [cooler.set_func()](https://www.tarantool.io/docs/tdb/ru/3_x/reference/api_reference/cooler#reference_lua-cooler-set_func).
...
2. На вкладке **Configuration** добавьте в секцию экземпляров хранилищ (`storages`) технологическую роль `roles.cooler`
   ...
   Полный список опций конфигурации для роли `roles.cooler` можно найти в [Справочнике по конфигурации](https://www.tarantool.io/docs/tdb/ru/3_x/reference/configuration_reference#configuration_reference-cooler).
...
Просмотреть текущую статистику архивации можно с помощью методов [cooler.info()](https://www.tarantool.io/docs/tdb/ru/3_x/reference/api_reference/cooler#reference_lua-cooler-info) и
[cooler.stats()](https://www.tarantool.io/docs/tdb/ru/3_x/reference/api_reference/cooler#reference_lua-cooler-stats).
Для этого:
...
3. Вызовите метод `cooler.info()`:
   ...
   - `vinyl_params` — параметры спейса vinyl (см. параметры в [cooler.setup()](https://www.tarantool.io/docs/tdb/ru/3_x/reference/api_reference/cooler#reference_lua-cooler-setup)).
   ...
   - `resetup_count` — количество вызовов метода [cooler.resetup()](https://www.tarantool.io/docs/tdb/ru/3_x/reference/api_reference/cooler#reference_lua-cooler-resetup) для спейса.
...
Полный список доступных метрик модуля можно найти в разделе [Метрики Tarantool DB](https://www.tarantool.io/docs/tdb/ru/3_x/admin_guide/monitoring/metrics_tdb#admin_guide-monitoring_tdb-cooler).
...
Смотрите также: [Мониторинг в Tarantool DB](https://www.tarantool.io/docs/tdb/ru/3_x/admin_guide/monitoring).
...
2. Добавьте новую миграцию `002_sessions_resetup.lua`:
   ...
   В миграции используется метод [cooler.resetup()](https://www.tarantool.io/docs/tdb/ru/3_x/reference/api_reference/cooler#reference_lua-cooler-resetup), который при смене формата исходного спейса
