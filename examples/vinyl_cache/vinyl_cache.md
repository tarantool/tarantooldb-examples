...
Стенд состоит из одного инстанса Tarantool DB. Конфигурация хранится в etcd;
...
Измерим чтение 5 000 ключей и посмотрим счётчики:
...
Будем многократно читать один и тот же небольшой набор ключей (1–5000).
...
Будем читать случайные ключи по **всему** датасету (200 000 ключей, ~40 MB).
При кэше 4 MiB данные постоянно вытесняются, и кэш почти не помогает.

`bench_scattered` делает 50 000 случайных чтений по всему диапазону ключей — аналогична `bench_hot`
...
Время практически то же самое. Это показывает: кэш, который меньше рабочего набора данных,
не даёт выигрыша — его поведение неотличимо от полного отсутствия кэша.

Вывод: кэш кортежей эффективен только тогда, когда есть **горячий набор** данных,
целиком помещающийся в кэш. При равномерных обращениях ко всему датасету, превышающему
