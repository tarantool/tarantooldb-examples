...
в примерах документации. Для целевого развертывания используйте [инсталлятор Ansible Tarantool Enterprise](install_guide-ate).
```
...
- [/healthcheck](admin_guide-tdb_worker-healthcheck) -- проверка работоспособности узла обработчика;
- [/metrics](admin_guide-tdb_worker-metrics) -- метрики экземпляра обработчик;
- [/info](admin_guide-tdb_worker-info) -- общая информация об обработчике.
...
Подробная информация о мониторинге в Tarantool DB и поддерживаемых метриках приведена в разделе [](admin_guide-monitoring).
