...
При создании архива приложения tt CLI генерирует дополнительные файлы с хешами всех компонентов приложения с помощью опции `--with-integrity-check`. В дальнейшем при запуске Tarantool DB будет отслеживать целостность всех компонентов приложения во время работы.

Содержание:

* [Пререквизиты](#пререквизиты)
* [Используемые файлы](#используемые-файлы)
* [Создание ключевой пары](#создание-ключевой-пары)
* [Упаковка приложения](#упаковка-приложения)
* [Установка приложения](#установка-приложения)
* [Запуск в режиме проверки целостности](#запуск-в-режиме-проверки-целостности)
* [Проверка работы контроля целостности](#проверка-работы-контроля-целостности)
* [Контроль целостности при использовании централизованного хранилища конфигурации](#контроль-целостности-при-использовании-централизованного-хранилища-конфигурации)
...
Для упаковки приложения с опцией `--with-integrity-check` сначала необходимо создать ключевую пару:

```shell
cd examples/integrity_check
...
Загрузите архив для развёртывания Tarantool DB и распакуйте его в папку примера `examples/integrity_check`:
...
Пример: `tarantooldb-3.2.0.linux.x86_64.tar.gz`.
...
Подробнее о запуске с проверкой целостности можно также узнать в документации tt CLI в [соответствующем разделе](https://www.tarantool.io/ru/doc/latest/tooling/tt_cli/start/#integrity-check).
...
Перейдите в папку распакованного приложения `examples/integrity_check/tarantooldb_ic`.
