...
Теперь обратим внимание на график `Tarantool replication lag`:

(Для наглядности размер оси Y выставлен до 40ms)

Можно заметить небольшое расхождение при синхронной репликации
![](./images/replication_lag_sync.png)
Как можно заметить фоновое значение колеблется в пределах 40ms. Синхронная репликация чуть медленнее отрабатывает. Это связано с тем, что ко времени коммита прибавляется replication_lag.

Но если запустить запись в асинхронный спейс, можно заметить, что лаг стал чуть меньше:
![](./images/replication_lag_async.png)
Его фон не превышает 10ms.

В клиентском приложении есть константа `STREAMS = 120`. При увеличении количества потоков увеличится и нагрузка, а как следствие увеличится расхождение по replication_lag.
