...
Как видно, фоновое значение Tarantool replication lag колеблется в пределах 40ms (для наглядности размер оси Y выставлен до 40ms). Синхронная репликация работает немного медленнее из-за добавления времени replication_lag к времени коммита.
...
В этом случае, фоновое значение Tarantool replication lag не превышает 10ms, что свидетельствует о снижении лага при асинхронной репликации.
