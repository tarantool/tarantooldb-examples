...
> (переменной `TARANTOOL_SYNCHRO_TIMEOUT`) значение в 86900 или больше.
> По умолчанию в режимах `election_mode = 'manual'` и `election_mode = 'off'` задано значение в 5 секунд.
> Если в режиме `manual` увеличить значение параметра `replication_synchro_timeout` до 86900 или более, синхронные транзакции не будут откатываться по истечении времени ожидания.
