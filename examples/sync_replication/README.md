...
## {heading(Предотвращение split-brain при выборе нового лидера)[id=user_guide-sync_replication-split_brain]}
...
> В этом случае синхронные транзакции не будут откатываться по истечении времени ожидания. В режиме `election_mode = 'off'` значение по умолчанию составляет 5 секунд.
> 
> Когда связь с новым мастер-узлом будет восстановлена и старый мастер-узел узнает о новом мастер-экземпляре, синхронные транзакции все равно откатятся.
