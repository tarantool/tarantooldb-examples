...
* [Перевод репликасетов в election\_mode = manual](#перевод-репликасетов-в-election_mode--manual)
...
Синхронная репликация гарантирует, что транзакция считается успешно завершенной только после того, как запись в WAL выполнена не только на лидере, но и подтверждена кворумом реплик.

Для корректной работы автоматического восстановления после отказов (failover) и захвата синхронной очереди в Tarantool DB должны быть соблюдены условия:
