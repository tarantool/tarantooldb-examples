...
> [!IMPORTANT]
>  Ограничения:
>  * Режим Stateful failover не поддерживает работу с наборами реплик, у которых включена опция `all_rw = true`, а также с наборами реплик, состоящими из одного узла.
>  * Если автоматическое восстановление после отказов отключено, для корректной работы с синхронными спейсами потребуется вручную вызывать `box.ctl.promote()` на новых лидерах наборов реплик. 
...
При выполнении миграции был создан синхронный спейс `orders`(для него задано значение `is_sync = true`). Однако по умолчанию после старта кластера **системные спейсы** создаются в асинхронном режиме.

Чтобы перевести системные спейсы в синхронный режим, отправьте POST-запрос на HTTP-адрес обработчика запросов (*endpoint*) [/sync](https://www.tarantool.io/docs/tdb/ru/1_x/reference/http_reference#reference-http-sync) любого узла – например, на порт 8081):

```shell
curl -X POST "http://localhost:8081/sync"
```

При успешном выполнении команда вернет JSON-ответ со статусом `success` для хранилищ и списком синхронных спейсов:
...
>  При восстановлении связи это приведет к ошибке `ER_SPLIT_BRAIN`, для устранения которой потребуется ручная повторная начальная загрузка (*rebootstrap*) старого лидер-узла.
...
Поскольку спейс `orders` работает в синхронном режиме, операция записи завершается только после того, как запись в WAL подтверждена кворумом реплик.

## Проверка устойчивости синхронной репликации при отказе узлов

Чтобы убедиться, что синхронная репликация гарантирует сохранность данных и предотвращает запись при отсутствии связи с большинством узлов, протестируйте поведение кластера при потере кворума.
...
По умолчанию восстановление после отказа в режиме `stateful` использует режим выбора лидера `election_mode = 'off'`, полагаясь при смене лидер-узла на сравнение `vclock`.
...
> [!WARNING]
> При разделении сети и выборе нового лидера в режиме `election_mode = 'manual'` может возникнуть *split-brain* — появление **двух и более** лидер-узлов в одном наборе реплик. Чтобы этого избежать, задайте для параметра `replication_synchro_timeout`
> (переменной `TARANTOOL_REPLICATION_SYNCHRO_TIMEOUT`) значение в 86400 или больше.
> По умолчанию в режимах `election_mode = 'manual'` и `election_mode = 'off'` задано значение в 5 секунд.
> Если в режиме `manual` увеличить значение параметра `replication_synchro_timeout` до 86400 или более, синхронные транзакции не будут откатываться по истечении времени ожидания.
> Когда связь с новым мастер-узлом будет восстановлена и старый мастер-узел узнает о новом мастер-экземпляре, неподтвержденные синхронные транзакции все равно откатятся.
...
> [!WARNING]
>  Эти хелперы применяют настройки динамически, но не являются персистентными. Перед будущими перезапусками узлов необходимо явно переопределить переменные окружения `TARANTOOL_ELECTION_MODE` и `TARANTOOL_ELECTION_FENCING_MODE`.
