...
Узнать больше: [Начало работы с модулем dictionary](https://www.tarantool.io/docs/tdb/ru/2_x/user_guide/dictionary/dictionary_gs).
...
> [!IMPORTANT]
> Если используется Tarantool DB версии ниже 2.2.0, для работы с данными словарей через API требуется выполнить дополнительный шаг — назначить роль `dictionary_api_service` пользователю кластера, который указан в разделе конфигурации [iproto.advertise.peer](https://www.tarantool.io/ru/doc/latest/reference/configuration/configuration_reference/#configuration-reference-iproto-advertise-peer). Пример:
>
> ```yaml
> credentials:
>   users:
...
>  * Отдельный архив [dictionary_user.zip](https://download-directory.github.io/?url=https%3A%2F%2Fgithub.com%2Ftarantool%2Ftarantooldb-examples%2Ftree%2Frelease-2x%2Fmaster%2Fexamples%2Fdictionary_user&filename=dictionary_user), скачанный из этого репозитория.
...
Добавьте несколько элементов с соответствующими им ключами в словарь `categories` с помощью метода [dictionary_router_set()](https://www.tarantool.io/docs/tdb/ru/2_x/reference/api_reference/dictionary#reference_lua-dictionary-set):
...
Проверьте записанные в словарь данные с помощью метода [dictionary_router_get()](https://www.tarantool.io/docs/tdb/ru/2_x/reference/api_reference/dictionary#reference_lua-dictionary-get):
